...
export function buildEatList(params) {
    ...
    for (const prey of candidates) {
        ...
        if (prey.kind === "food") {
            ...
            if (dx * dx + dy * dy + 1 > eaterSquare)
                continue;
            out.push({ preyId: prey.id });
            continue;
        }
        const preyMassForSquare = prey.kind === "virus" ? prey.sizeMass : prey.kind === "player" ? prey.mass : prey.mass;
        const preySquare = massToSquareSize(preyMassForSquare);
        if (dx * dx + dy * dy + preySquare > eaterSquare)
            continue;
