 ...
 */
function getUsedDepositsKey(serverId) {
 ...
 */
export async function getUsedDeposits(serverId) {
    const redis = getRedisClient();
    const key = getUsedDepositsKey(serverId);
 ...
 */
export async function unmarkDepositUsed(serverId, depositId) {
    const redis = getRedisClient();
    const key = getUsedDepositsKey(serverId);
 ...
 */
export async function closeDepositTracker() {
    if (redisClient) {
