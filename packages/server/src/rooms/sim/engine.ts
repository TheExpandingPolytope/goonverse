...
export type EngineEvent =
  ...
  | { type: "foodRemoved"; mass: number } // removed without being eaten (refund candidate)
  | { type: "ejectedRemoved"; mass: number } // removed without being eaten (refund candidate)
  | { type: "massDecayed"; mass: number } // decayed from players (refund candidate)
  | { type: "ejectedFedVirus"; mass: number }; // ejected fed to virus (refund candidate)
...
export class FfaEngine {
  ...
  addPlayer(params: { sessionId: string; wallet: `0x${string}`; displayName: string; spawnMass: number }): PlayerSim {
  ...
  getPlayerTotalMass(sessionId: string): number {
    ...
    for (const id of p.cellIds) {
      const n = this.nodes.get(id);
      if (n && n.kind === "player") total += n.mass;
    }
    return total;
  }
  ...
  setInput(sessionId: string, input: Partial<PlayerSim["input"]>) {
    ...
    if (input.splitPressed) player.input.splitPressed = true;
    if (input.ejectPressed) player.input.ejectPressed = true;
  }
  ...
  step(): EngineTickResult {
    ...
    for (const id of [...this.playerNodeIds]) {
      ...
      const ownedCells: PlayerNode[] = [];
      for (const cid of player.cellIds) {
        const c = this.nodes.get(cid);
        if (c && c.kind === "player") ownedCells.push(c);
      }
    ...
    for (let i = 0; i < this.movingNodeIds.length; i++) {
      ...
      const move = (node as PlayerNode | EjectedNode).move;
  ...
  private splitAllEligible(player: PlayerSim) {
  ...
  private ejectAllEligible(player: PlayerSim) {
    const current = [...player.cellIds];
    for (const id of current) {
      const node = this.nodes.get(id);
      if (!node || node.kind !== "player") continue;
  ...
  private consume(eater: PlayerNode, prey: WorldNode, events: EngineEvent[]) {
    // Ensure prey still exists
    if (!this.nodes.has(prey.id)) return;
  ...
  private spawnVirusedCell(player: PlayerSim, parent: PlayerNode, angleRad: number, mass: number, speed: number) {
    const child: PlayerNode = {
      id: this.newNodeId(),
      kind: "player",
  ...
  private removeFromList(list: number[], id: number) {
  ...
  private trySpawnVirus() {
    ...
    for (const id of this.virusNodeIds) {
      const node = this.nodes.get(id);
      if (!node || node.kind !== "virus") continue;
  ...
  private tryFeedVirus(ejected: EjectedNode, events: EngineEvent[]): boolean {
    ...
    let target: VirusNode | null = null;
    for (const id of this.virusNodeIds) {
      const node = this.nodes.get(id);
      if (!node || node.kind !== "virus") continue;
      if (node.y > bottomY) continue;
      if (node.y < topY) continue;
      if (node.x > rightX) continue;
      if (node.x < leftX) continue;
      target = node;
      break;
    }
  ...
  private shootVirus(parent: VirusNode) {
    const v: VirusNode & { move?: MoveEngine } = {
      id: this.newNodeId(),
      kind: "virus",
      x: parent.x,
      y: parent.y,
  ...
  private oneSecondUpdate(events: EngineEvent[]) {
