 ...
 */
export function processEating(
  state: GameState,
  blobGrid: SpatialGrid<BlobEntity>,
  pelletGrid: SpatialGrid<Pellet>,
  ejectedMassGrid: SpatialGrid<EjectedMass>
): string[] {
  const killedPlayers: string[] = [];
  const eatenPellets: string[] = [];
  const eatenEjectedMass: string[] = [];
  const eatenBlobs: Set<string> = new Set();
  ...
  state.players.forEach((player) => {
    ...
    for (const blob of player.blobs) {
      ...
      for (const otherEntity of nearbyBlobs) {
        if (eatenBlobs.has(otherEntity.id)) continue;
        if (otherEntity.blob.owner === blob.owner) continue; // Same player
      ...
      for (const pellet of nearbyPellets) {
        if (eatenPellets.includes(pellet.id)) continue;
 ...
 */
function eatBlob(eater: Blob, prey: Blob): void {
 ...
 * Blob eats a pellet
 */
function eatPellet(blob: Blob, pellet: Pellet): void {
 ...
 */
function eatEjectedMass(blob: Blob, ejected: EjectedMass): void {
 ...
 */
export function updatePlayerMass(player: Player): void {
  let totalMass = 0;
  for (const blob of player.blobs) {
    totalMass += blob.mass;
  }
