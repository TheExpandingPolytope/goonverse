 ...
 */
export function spawnPellets(state: GameState, deltaTime: number, balanceSystem: BalanceSystem): Pellet[] {
  ...
  const toSpawn = Math.min(
    Math.ceil(pelletsPerTick),
    GAME_CONFIG.MAX_PELLETS - currentCount
  );
 ...
 */
export function initializePellets(state: GameState, percentage: number = 0.5): void {
 ...
 */
export function removePellet(state: GameState, pelletId: string): void {
