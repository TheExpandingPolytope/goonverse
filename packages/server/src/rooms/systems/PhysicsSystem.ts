 ...
 */
export function updateBlobMovement(blob: Blob, deltaTime: number): void {
 ...
 */
function updateExitingBlobMovement(blob: Blob, deltaTime: number): void {
  // Calculate direction to target
  const dx = blob.targetX - blob.x;
  const dy = blob.targetY - blob.y;
  const distToTarget = vectorLength(dx, dy);
 ...
 */
export function updateEjectedMassMovement(ejectedMass: EjectedMass, deltaTime: number): void {
 ...
 */
function clampToWorld(blob: Blob): void {
 ...
 */
function clampEjectedMassToWorld(ejectedMass: EjectedMass): void {
 ...
 */
export function updateSplitTimer(blob: Blob, deltaTimeMs: number): void {
 ...
 */
export function startExitHold(blob: Blob): void {
  blob.isExiting = true;
  blob.exitProgress = 0;
 ...
 */
export function cancelExitHold(blob: Blob): void {
 ...
 */
export function updateExitProgress(blob: Blob, exitStartedAt: number, exitHoldMs: number): boolean {
 ...
 */
export function applySoftCollision(player: Player): void {
 ...
 */
function resolveSoftCollision(blobA: Blob, blobB: Blob): void {
 ...
 */
export function applyAttraction(player: Player, deltaTime: number): void {
  if (player.blobs.length < 2) return;
