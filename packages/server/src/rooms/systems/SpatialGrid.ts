...
export class SpatialGrid<T extends SpatialEntity> {
  ...
  constructor(
    worldWidth: number = GAME_CONFIG.WORLD_WIDTH,
    worldHeight: number = GAME_CONFIG.WORLD_HEIGHT,
    cellSize: number = GAME_CONFIG.GRID_CELL_SIZE
  ) {
   ...
   */
  getAllEntities(): T[] {
    const allEntities: Set<T> = new Set();
